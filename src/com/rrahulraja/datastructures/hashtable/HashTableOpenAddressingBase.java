...
/**
 * Base class for hashtables with open addressing collision resolution method
 * such as liner probing, quadratic probing and double hashing
 * 
 * @author Rrahul
 *
 * @param <K>
...
public abstract class HashTableOpenAddressingBase<K, V> implements Iterable<K> {
	...
	// occur for whatever open addressing scheme we are implementing
	...
	// Returns a list of non-unique values found in hash-table
	...
	protected void resizeTable() {
		...
		// re-insert all keys into the hash-table
		keyCount = usedBuckets = 0;
		...
		for (int i = offset, j = -1, x = 1;; i = normalizeIndex(offset + probe(x++))) {
			...
			if (keys[i] == TOMBSTONE) {
				...
				if (j == -1) {
					...
					// the new element a i where the null element is insert
